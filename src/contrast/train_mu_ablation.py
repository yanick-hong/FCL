...
def parse_args():
    ...
    p.add_argument("--best_ckpt", type=str, default=str(experiment_checkpoint("CIFAR100_mu_ablation")))
    p.add_argument("--val_dir", type=str, default=str(experiment_logs("CIFAR100_mu_ablation")))
...
# ----------------------- AUC（引入 mu 消融） -----------------------
def auc_noise_only_a11_loss(logits, clip_probs, s, mu=0.5, tau=0.5, eps: float = 1e-12):
    """
    OVR pairwise logistic AUC；实现公式: q = mu * p_model + (1 - mu) * p_clip
    """
    ...
    lo = logits[idx]                          
    p_clip = clip_probs[idx].detach()         
    p_model = F.softmax(lo, dim=-1)         
    
    # 核心改动：使用传入的 mu 参数进行凸组合 
    q = mu * p_model + (1.0 - mu) * p_clip          
    ...
    for i in range(C):
        ...
        ell_posneg = F.softplus(-diff / tau)  
...
def train(
    ...
    set_seed(42); lam = float(lam); mu = float(mu)
    ...
    print(f"[INFO] Ntrain={N}, Nval={val_labels.numel()}, D={D}, C={C}")
    print(f"[ABLATION] lam={lam:.2f}, mu={mu:.4f} | Loss = CE + lam * AUC_C(mu)")
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for ib, batch in enumerate(train_loader, start=1):
            ...
            f = f.to(device); y_true = y_true.to(device); s = s.to(device); p_clip = p_clip.to(device)

            logits = head(f); mask_t = (s == 0)

            ce_trusted = ce_criterion(logits[mask_t], y_true[mask_t]) if mask_t.any() else logits.sum()*0.0

            # 调用带 mu 的 AUC 计算 
            auc_untrusted = auc_noise_only_a11_loss(
                logits=logits, clip_probs=p_clip, s=s, mu=mu, tau=0.5
            )
            ...
            if ib % print_every == 0:
                print(f"[Epoch {epoch:03d}] {ib}/{len(train_loader)} | mu={mu:.2f} | CE={ce_trusted.item():.4f} | AUC={auc_untrusted.item():.4f}")

        val_loss, val_acc = evaluate(head, val_feats, val_labels)
        print(f"[VAL] epoch={epoch:03d} | val_acc={val_acc*100:.2f}%")
        ...
        if val_acc >= getattr(stopper, "best", -float('inf')) and best_ckpt_path:
            torch.save(head.state_dict(), best_ckpt_path)

        if stopper.step(val_loss, val_acc): break

if __name__ == "__main__":
    args = parse_args(); save_experiment_config("CIFAR100_mu_ablation", args); set_seed(args.seed)
    train(
        ...
        max_epochs=args.max_epochs, lam=args.lam, mu=args.mu,
        print_every=args.print_every, best_ckpt_path=args.best_ckpt,
        patience=args.patience, min_delta=args.min_delta, val_dir=args.val_dir,  
    )
